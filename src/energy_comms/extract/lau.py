...
def extract_lau():
    ...
    for file in FILE_NAMES:
        ...
        df_areas = df_areas.loc[
            df_areas["area_code"].str.contains("CN")
            | df_areas["area_code"].str.contains("MT")
        ]
        df_areas.reset_index()

        df_areas["geographic_level"] = np.where(
            df_areas["area_code"].str.contains("CN"), "county", "metropolitan_stat_area"
        )

        df_areas["state_code"] = df_areas["area_code"].str[2:4]

        df_areas["geoid"] = np.where(
            df_areas["geographic_level"] == "county",
            df_areas["area_code"].str[4:7],
            df_areas["area_code"].str[4:10],
        )

        df_areas["series_id"] = (
            "LAU"
            + df_areas["area_code"].str[0:2]
            + df_areas["state_code"]
            + df_areas["geoid"]
        )

        df_areas["series_id"] = np.where(
            df_areas["geographic_level"] == "county",
            df_areas["series_id"] + "00000000",
            df_areas["series_id"] + "00000",
        )
...
def get_lau_data(lau_file):
    ...
    df = df.query(
        "geographic_level== 'metropolitan_stat_area' | geographic_level == 'county'"
    )
...
def get_data():
    ...
    for year in years:
        ...
        year_file_combination = {
            "2010": "la.data.0.CurrentU10-14.txt",
            "2015": "la.data.0.CurrentU15-19.txt",
            "2020": "la.data.0.CurrentU20-24.txt",
        }
