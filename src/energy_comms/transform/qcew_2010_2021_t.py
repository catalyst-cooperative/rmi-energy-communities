...
from energy_comms.extract.qcew_2014_2021 import (
    get_2014_industry_with_bls_geographic_tag as geo_tag,
)
...
def transform(df):
    ...
    total_employment = total_employment.rename(
        columns={"annual_avg_emplvl": "total_employees"}
    )
    total_employment_df = total_employment[
    ...
    total_fossil_employment = (
        fossil_employment.groupby(
            ["area_fips", "area_title", "geographic_level", "year"]
        )
        .agg({"annual_avg_emplvl": "sum"})
        .reset_index()
    )
    total_fossil_employment = total_fossil_employment.rename(
        columns={"annual_avg_emplvl": "fossil_employees"}
    )
    total_fossil_employment_df = total_fossil_employment[
        ["area_fips", "area_title", "geographic_level", "year", "fossil_employees"]
    ]

    # combine for total and fossil 2010-2013
    combined = total_employment_df.merge(
        total_fossil_employment_df,
    ...
    combined["geoid"] = np.where(
        combined["geographic_level"] == "metropolitan_stat_area",
        combined["geoid"] + "0",
        combined["geoid"],
    )
    # only keep geo levels we need (e.g. remove state and country totals)
    final_employment = combined.query(
        "geographic_level=='county' or geographic_level=='metropolitan_stat_area'"
    )
    ...
    employment["fips_county"] = np.where(
        employment["geographic_level"] == "county",
        employment["geoid"],
        employment["FIPS code"] + employment["County code"],
    )

    # create eligibility criteria # 1) % fossil employees
    employment["percent_fossil_employment"] = (
        employment["fossil_employees"] / employment["total_employees"] * 100
    )
    employment["meets_fossil_threshold"] = np.where(
        employment["percent_fossil_employment"] > 0.17, 1, 0
    )
    ...
    unemployment = local_unemployment.merge(
        national_unemployment, on="year", how="left"
    )

    # make column a float for comparison
    unemployment["local_area_unemployment"] = pd.to_numeric(
        unemployment["local_area_unemployment"], errors="coerce"
    )

    # fix geoid for merge with employment

    unemployment["geoid"] = np.where(
        unemployment["geographic_level"] == "county",
        unemployment["state_code"] + unemployment["geoid"],
        unemployment["geoid"].str[0:5],
    )

    unemployment["year"] = unemployment["year"].astype(str)

    employment_both = employment.merge(
        unemployment, on=["geoid", "geographic_level", "year"], how="inner"
    )
    employment_both["meets_unemployment_criteria"] = np.where(
        employment_both["local_area_unemployment"]
        > employment_both["national_unemployment_rate_prev_year"],
        1,
        0,
    )
    ...
    eligible_employment_areas = employment_both.query(
        "meets_fossil_threshold==1 & meets_unemployment_criteria==1"
    )

    # clean for export
    eligible_employment_areas = eligible_employment_areas.rename(
        columns={"May 2021 MSA name": "msa_name"}
    )
    eligible_employment_areas = eligible_employment_areas[
        [
            "area_title",
            "geographic_level",
            "State",
            "msa_name",
            "geoid",
            "meets_fossil_threshold",
            "local_area_unemployment",
            "national_unemployment_rate_prev_year",
            "meets_unemployment_criteria",
            "fips_county",
        ]
    ]
